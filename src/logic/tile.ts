import { Dir, left, right, back } from './dir';

export type Tile = { code: number, name: string, arrow: number };

function makeTile(code: number, name: string, arrow: number = 0): Tile {
  return { code, name, arrow };
}
...
export function isDirectionalForce(code: number) {
...
export function isForce(code: number) {
  return code === CC.FORCE_RANDOM.code || isDirectionalForce(code);
}

export function isIce(code: number) {
  return code === CC.ICE.code || isIceCorner(code);
}

function isIceCorner(code: number) {
...
function isSingleThinWall(code: number) {
  return code >= CC.THIN_WALL_N.code && code <= CC.THIN_WALL_E.code;
}

export function isUnused(code: number) {
...
function dirFromCode(code: number) {
...
function _dirFromCode(code: number, base?: number) {
...
function _dirFromForce(code: number) {
  switch (code) {
...
function codeForDir(code: number, dir: number) {
...
function _codeForDir(code: number, dir: number, base?: number) {
...
function _forceForDir(dir: number) {
  switch (dir) {
...
export function monsterTypeFromCode(code: number) {
...
export function flipTileHorizontally(code: number) {
...
export function flipTileVertically(code: number) {
  if (isDirectional(code)) {
    const dir = dirFromCode(code);
...
export function rotateTileLeft(code: number) {
  if (isDirectional(code)) {
    const dir = dirFromCode(code);
...
export function rotateTileRight(code: number) {
  if (isDirectional(code)) {
    const dir = dirFromCode(code);
