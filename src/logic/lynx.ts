...
import { DEFAULT_LEVEL_SIZE, EndReasons, pointForLayerIndex } from '../util/utils';
...
export class LynxLogic {
  ...
  */
  ...
  setfloorat(pos: number, id: number) {
    this.state.map[pos].top = id;
    if (this.state.mapUpdatedCallback) this.state.mapUpdatedCallback(pos, id);
  }
  ...
  */
  removecreature(cr: Creature, animationid: number) {
    ...
    if (this.state.removeCreatureCallback) this.state.removeCreatureCallback(cr, animationid);
  }
  ...
  */
  canpushblock(block: Creature, dir: number, flags: number) {
    ...
    if (!this.canmakemove(block, dir, flags)) {
      if (!block.moving && (flags & (CMM_PUSHBLOCKS | CMM_PUSHBLOCKSNOW))) block.dir = dir;
      return false;
    }
    if (flags & (CMM_PUSHBLOCKS | CMM_PUSHBLOCKSNOW)) {
      ...
      if (flags & CMM_PUSHBLOCKSNOW) this.advancecreature(block, false);
    }
  ...
  */
  canmakemove(cr: Creature, dir: number, flags: number) {
    ...
    if (x < 0 || x >= LEVEL_SIZE) return false;
    if (y < 0 || y >= LEVEL_SIZE) {
      return false;
    }
    ...
    if (cr.id === TW.Chip) {
      ...
      if (floor === TW.HiddenWall_Temp || floor === TW.BlueWall_Real) {
        if (flags & CMM_STARTMOVEMENT) this.setfloorat(to, TW.Wall);
        return false;
      }
  ...
  */
  choosechipmove(cr: Creature, discard: boolean) {
    ...
    if (cr.tdir !== Dir.NONE) dir = cr.tdir;
    else if (getfdir(cr) !== Dir.NONE) dir = getfdir(cr);
    else return;
    ...
    if (isdiagonal(dir)) {
      ...
      } else if (this.canmakemove(cr, dir & (Dir.E | Dir.W), CMM_PUSHBLOCKS)) dir &= Dir.E | Dir.W;
      else dir &= Dir.N | Dir.S;
      cr.tdir = dir;
    } else {
  ...
  */
  choosemove(cr: Creature) {
    if (cr.id === TW.Chip) {
      ...
      if (cr.tdir === Dir.NONE && getfdir(cr) === Dir.NONE) this.resetfloorsounds(false);
    } else if (this.getforcedmove(cr)) cr.tdir = Dir.NONE;
  ...
  */
  teleportcreature(cr: Creature) {
    ...
    for (;;) {
      --pos;
      if (pos < 0) pos += LEVEL_SIZE * LEVEL_SIZE;
      if (this.floorat(pos) === TW.Teleport) {
        if (cr.id !== TW.Chip) this.removeclaim(cr.pos);
        cr.pos = pos;
        if (!this.islocationclaimed(pos) && this.canmakemove(cr, cr.dir, 0)) break;
        if (pos === origpos) {
          if (cr.id === TW.Chip) this.state.stuck = true;
          else this.claimlocation(cr.pos);
          return false;
        }
      } else if (this.ismarkedteleport(pos)) {
        this.setfloorat(pos, TW.Teleport);
        if (pos === this.chippos()) this.getchip().hidden = true;
      }
    }
  ...
  */
  springtrap(pos: number) {
    ...
    const cr = this.lookupcreature(pos, true);
    if (cr && cr.dir !== Dir.NONE) this.advancecreature(cr, true);
  }
  ...
  */
  startmovement(cr: Creature, releasing: boolean) {
    ...
    if (cr.id === TW.Chip) {
      if (!cr.hasForceboots) {
        if (isslide(floorfrom) && cr.tdir === Dir.NONE) cr.state |= CS_SLIDETOKEN;
        else if (!isice(floorfrom) || cr.hasSkates) cr.state &= ~CS_SLIDETOKEN;
      }
    }
    ...
    if (cr.id !== TW.Chip) {
      ...
      if (cr.id !== TW.Block && cr.pos === this.state.chiptopos) this.state.chiptocr = cr;
    ...
    cr.pos += delta[dir];
    if (cr.id !== TW.Chip) this.claimlocation(cr.pos);
    // if (cr.id === TW.Chip && this.state.chipMovedCallback) this.state.chipMovedCallback(cr.pos);
    cr.moveStartTimestamp = Date.now();
  ...
  */
  continuemovement(cr: Creature): boolean {
    ...
    const floor = this.floorat(cr.pos);
    if (isslide(floor) && (cr.id !== TW.Chip || !cr.hasForceboots)) speed *= 2;
    else if (isice(floor) && (cr.id !== TW.Chip || !cr.hasSkates)) speed *= 2;
    cr.moving -= speed;
  ...
  */
  advancecreature(cr: Creature, releasing: boolean) {
    ...
    if (!this.continuemovement(cr)) {
      if (!this.endmovement(cr, false)) return -1;
    }
  ...
  */
  initialhousekeeping() {
    ...
    const chip = this.getchip();
    if (chip && chip.id === TW.Pushing_Chip) chip.id = TW.Chip;
    ...
    for (const cr of this.state.creatures) {
      ...
      if (cr.state & CS_REVERSE) {
        cr.state &= ~CS_REVERSE;
        if (cr.moving <= 0) cr.dir = back(cr.dir);
      }
    }
    for (const cr of this.state.creatures) {
      if (cr.state & CS_PUSHED) {
  ...
  */
  preparedisplay() {
    ...
    if (!chip.hidden) {
      if (floor === TW.HintButton && chip.moving <= 0) this.showhint();
      else this.hidehint();
      if (chip.id === TW.Chip && this.state.pushing) chip.id = TW.Pushing_Chip;
      if (chip.moving) {
        this.resetfloorsounds(false);
  ...
  */
  advancegame(): number {
    ...
    const cr = this.getchip();
    if (cr) {
      if (getfdir(cr) === Dir.NONE && cr.tdir === Dir.NONE) this.state.couldntmove = false;
      else this.checkmovingto();
    }
    ...
    for (let i = this.state.creatures.length - 1; i >= 0; i--) {
      ...
      // }
      if (this.floorat(cr.pos) === TW.Button_Brown && cr.moving <= 0) this.springtrap(this.trapfrombutton(cr.pos));
    }
